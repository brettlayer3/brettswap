...
import { BSWAP_DISTRIBUTOR_ADDRESS, BSWAP_VAULT_ADDRESS } from '../../constants/addresses'
...
const VaultListItem = ({ farm }) => {
  ...
  const [approvalState, approve] = useApproveCallback(typedDepositValue, BSWAP_VAULT_ADDRESS[chainId])
      ...
      <Modal isOpen={showConfirmation} onDismiss={() => setShowConfirmation(false)}>
        <div className="space-y-4">
          ...
          <Typography variant="lg" className="font-medium pt-4">
            {i18n._(
              t`${
                currentAction.action == 'deposit' ? 'Staking on Bswap Vault' : 'Harvesting'
