...
export function useUserInfo(farm, token) {
  ...
  const contract = useBswapVaultContract()
...
export function useBswapPositions(contract?: Contract | null) {
  ...
  return useMemo(() => {
    if (!pendingBswap || !userInfo || !userLockedUntil) {
      return []
    }
    return zip(pendingBswap, userInfo, userLockedUntil)
      .map((data, i) => ({
        id: args[i][0],
  ...
  }, [args, pendingBswap, userInfo, userLockedUntil])
}

export function usePositions() {
  return useBswapPositions(useBswapVaultContract())
}

export function useBswapVaults(contract?: Contract | null) {
  const numberOfPools = useSingleCallResult(contract ? contract : null, 'poolLength', undefined, NEVER_RELOAD)
    ?.result?.[0]
...
export function useVaults() {
  return useBswapVaults(useBswapVaultContract())
}
...
export function useBswapVaultInfo() {
  return useVaultInfo(useBswapVaultContract())
}
