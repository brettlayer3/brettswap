...
import { useBswapContract } from '../../hooks'
...
export default function TokenStatsModal({ token }: { token: any }) {
  ...
  let tokenInfo = useTokenInfo(useBswapContract())

  if (token.symbol == 'BRETT') tokenInfo = { circulatingSupply: '1500000', burnt: '0', totalSupply: '0', vaults: '0' }

  const price = formatNumberScale(priceData?.[token.symbol.toLowerCase()], true, 2)

  const modalOpen = useModalOpen(token.symbol == 'BSWAP' ? ApplicationModal.BSWAP_STATS : ApplicationModal.BRETT_STATS)
            ...
            <div className="flex flex-1 flex-col">
              ...
              <div className="flex items-center justify-between space-x-3 gap-2">
                {token?.address && (
                  <ExternalLink
                    href={
                      'https://sepolia.brettscan.comtokens/0x6bD193Ee6D2104F14F94E2cA6efefae561A4334B'
                    }
