...
async function faucetSend(req) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    resolve({
      status: 200,
      message: `You will receive BRETT in your wallet soon.`,
    })
  })
}
...
async function checkBridgeUsage(address) {
  const bridges = {
    ETH: `https://bridgeapi.anyswap.exchange/v2/swapin/history/${address}/532/1/allv2?offset=0&limit=1`,
    BSC: `https://bridgeapi.anyswap.exchange/v2/swapin/history/${address}/532/56/allv2?offset=0&limit=1`,
  }
...
export default async function handler(req, res) {
  ...
  if (verified) {
    ...
    if (usedBridge) {
      const brettBalance = parseFloat(web3.utils.fromWei(await web3.eth.getBalance(address)))
      if (brettBalance < 0.001) {
        ...
        res.status(200).json({
          status: 403,
          message: 'Your current BRETT balance is above the minimum requirement to use the faucet.',
        })
      }
    } else {
      res.status(200).json({
